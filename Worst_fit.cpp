...
void sort(int** arr, int size)   //Sorting the array in decreasing order so that first fit is the largest memory chunk.
{
...
int worst_fit(int** arr, int size, int memory)
{
...
int main()
{
    ...
    for(int i = 0; i < number_of_processes; i++)
    {
        int x = worst_fit(memory, number_of_processes, processes[i]);
        if(x == -1)
        {
